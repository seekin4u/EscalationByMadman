...
/proc/get_id_photo(var/mob/living/carbon/human/H, var/assigned_role)
	...
	var/use_icon_path = 'icons/mob/human_face.dmi'
	var/obj/item/organ/external/head/temp_head = H.get_organ(BP_HEAD)
	if(temp_head)
		use_eye_icon = temp_head.getIconName()
		use_icon_path = temp_head.getIconPath()
	var/icon/eyes_s = new/icon("icon" = use_icon_path, "icon_state" = use_eye_icon)
