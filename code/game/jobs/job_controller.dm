...
/datum/controller/occupations
	...
	proc/SetupOccupations(var/faction = "Station", var/setup_titles = 0)
		...
		for(var/J in all_jobs)
			...
			if(job.department_flag & NATO)
				engineering_positions += job.title
			if(job.department_flag & WARPAC)
	...
	if(spawnpos && istype(spawnpos))
		if(spawnpos.check_job_spawning(rank))
			...
			if(return_location)
				...
				if(H)
					H << "Your chosen spawnpoint ([spawnpos.display_name]) is unavailable for your chosen job. Please select your spawn point in the preferences."
					return 0
	else
		if(return_location)
			return pick(latejoin)
