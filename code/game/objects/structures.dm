...
/obj/structure/attackby(obj/item/weapon/gun/W, mob/user)
	if(istype(W))
		if(W.mounted) return
		if(flags & CAN_MOUNT)
			if(user.a_intent == I_GRAB)
				user.visible_message("<span class='notice'>\The [user] begins to mount \the [W.name] on \the [src]!</span>",
	  	     	                       "<span class='notice'>You begin mounting \the [W.name] on \the [src].</span>")
				if(!do_after(user, 10, src))
					return
				user.visible_message("<span class='notice'>\The [user] mounts \the [W.name] on \the [src]!.</span>",
	   	   	                        "<span class='notice'>You mount \the [W.name] on \the [src].</span>")
				W.mounted = 1
				. = 1
				if(!do_after(user, INFINITY, progress = 0))
					W.mounted = 0
					return
				return 1
		else
