...
/obj/item/organ/proc/handle_antibiotics()
	var/antibiotics = 0
	if(owner)
		antibiotics = owner.reagents.get_reagent_amount("doxycicline")

	if (!germ_level || antibiotics < 5)
		return

	if (germ_level < INFECTION_LEVEL_ONE)
		germ_level = 6	//cure instantly
	else if (germ_level < INFECTION_LEVEL_TWO)
		germ_level -= 12	//at germ_level == 500, this should cure the infection in a minute
	else
		germ_level -= 24 //at germ_level == 1000, this will cure the infection in 5 minutes
