...
/datum/medical_effect/oxydeprivation
	name = "Oxygen deprivation"
	triggers = list("morphine" = 15)
	cures = list("naloxone")
	cure_message = "You can breath normally again."

/datum/medical_effect/cramps/on_life(mob/living/carbon/human/H, strength)
	switch(strength)
		if(1 to 10)
			H.custom_pain("There's kinda hard to breath.",0)
		if(11 to 30)
			H.custom_pain("You better find any sources of fresh air, quick!",0)
		if(31 to INFINITY)
			H.emote("me",1,"gasps!")
			H.custom_pain("You cannot even breath!",1)
			if(H.losebreath < 2)
				H.losebreath++
