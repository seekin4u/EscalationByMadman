	...
	/*if(istype(W, /obj/item/weapon/gun))
		...
		if(flags & CAN_MOUNT)
			if(user.a_intent == I_GRAB)
				user.visible_message("<span class='notice'>\The [user] begins to mount \the [G.name] on \the [src]!</span>",
	  	  	                          "<span class='notice'>You begin mounting \the [G.name] on \the [src].</span>")
				if(!do_after(user, 5, src))
					return
				user.visible_message("<span class='notice'>\The [user] mounts \the [G.name] on \the [src]!</span>",
	   	   	                        "<span class='notice'>You mount \the [G.name] on \the [src].</span>")
				G.mounted = 1
